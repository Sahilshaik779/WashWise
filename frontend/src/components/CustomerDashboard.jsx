  ...
  return (
    <div className="app-container">
              ...
              <div className="tab-content">
                  {activeOrders.length === 0 ? (
                      <div className="card no-data-card">
                          ...
                          <p>You currently don't have any active orders.</p>
                      </div>
                  ) : (
                          ...
                          {activeOrders.map((order) => (
                              <div className="card order-card" key={order.id}>
                                  <div className="card-header">
                                      <div>
                                          <h3>{order.name}</h3>
                                          <p className="order-id">ID: {order.id.substring(0, 12)}...</p>
                                      </div>
                                      <span className="status-badge" style={{backgroundColor: getStatusColor(order.status)}}>{getStatusText(order.status)}</span>
                                  </div>
                                  <div className="card-body">
                                      <div className="info-grid">
                                          <div className="info-item"><span>Date</span><strong>{new Date(order.created_at).toLocaleDateString()}</strong></div>
                                          <div className="info-item"><span>Items</span><strong>{order.clothes_count} pieces</strong></div>
                                      </div>
                                      <div className="progress-bar-container">
                                          <div className="progress-labels"><span>Progress</span><span>{getProgressPercentage(order.status)}%</span></div>
                                          <div className="progress-bar">
                                              <div className="progress-bar-fill" style={{width: `${getProgressPercentage(order.status)}%`, backgroundColor: getStatusColor(order.status)}}></div>
                                          </div>
                                      </div>
                                  </div>
                                  {order.qr_code_url && (
                                      <div className="card-footer">
                                          <p>Your QR Code for Pickup</p>
                                          <div className="qr-code-box" style={{borderColor: getStatusColor(order.status)}}>
                                              <img 
                                                src={`http://127.0.0.1:8000${order.qr_code_url}`} 
                                                alt="Order QR Code"
                                                className="qr-image"
                                                onClick={() => handleImageClick(order.qr_code_url)}
                                              />
                                          </div>
                                      </div>
                                  )}
              ...
              <div className="tab-content">
                  <div className="card">
                      {completedOrders.length === 0 ? (
                           <div className="no-data-card small">
                          ...
                          <div className="table-container">
                              <table className="data-table">
                                  <thead><tr><th>Order Name</th><th>Date</th><th>Items</th><th>Status</th></tr></thead>
                                  <tbody>
                                      {completedOrders.map(order => (
                                          <tr key={order.id}>
                                              <td>
                                                  <div className="order-name">{order.name}</div>
                                                  <div className="order-id">{order.id.substring(0, 12)}...</div>
                                              </td>
                                              <td className="text-center">{new Date(order.created_at).toLocaleDateString()}</td>
                                              <td className="text-center">{order.clothes_count}</td>
                                              <td><span className="status-badge" style={{backgroundColor: getStatusColor(order.status)}}>Completed</span></td>
                                          </tr>
                                      ))}
                                  </tbody>
                              </table>
                  ...
                  <div className="card">
                      ...
                      <form onSubmit={handleChangePassword}>
                          <div className="grid three-cols">
                              <div><label>Current Password:</label><input type="password" placeholder="Enter current password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} required/></div>
                              <div><label>New Password:</label><input type="password" placeholder="Enter new password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required/></div>
                              <div><label>Confirm New Password:</label><input type="password" placeholder="Confirm new password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required/></div>
                          </div>
      ...
      <style jsx>{`
        ...
        .sidebar { width: 260px; background-color: #242c3d; color: #fff; padding: 20px; flex-shrink: 0; }
        .content-area { flex-grow: 1; padding: 30px; overflow-y: auto; }
        .sidebar-nav { list-style: none; padding: 0; margin: 0; }
        .sidebar-nav button { display: flex; align-items: center; gap: 12px; width: 100%; padding: 12px 15px; margin-bottom: 8px; border-radius: 8px; background: transparent; border: none; color: rgba(255, 255, 255, 0.7); font-size: 1rem; font-weight: 500; cursor: pointer; text-align: left; transition: all 0.2s ease; }
        ...
        .orders-grid { display: grid; gap: 25px; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); }
        ...
        .qr-code-box { display: inline-block; padding: 10px; background-color: white; border-radius: 12px; border-width: 2px; border-style: solid; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        ...
        .btn-primary { padding: 12px 25px; background-color: #007bff; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 1rem; transition: all 0.2s ease; }
        .btn-primary:hover:not(:disabled) { transform: translateY(-2px); background-color: #0069d9; }
        ...
        .modal-image {
            width: 100%; max-width: 300px; height: auto; border-radius: 10px;
        }
        .modal-close {
            position: absolute;
            top: -18px; /* Half of height */
            right: -18px; /* Half of width */
            background-color: white; border: none; color: #333;
